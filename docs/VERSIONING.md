...
正式版本从 `v0.1.0` 开始。前端重构线在灰度期沿 `0.3.x` 演进，`v0.3.3` 于 2026-07-24 首次全量进入生产，并成为当前正式版本线。历史中另有一个与本次重构无关的误命名 `v0.3.0` tag，保留但不移动、不删除，也不作为后续版本依据。
...
| `v0.3.1` | 注册确认邮件链路与前端重构 Phase 0–2 灰度基线 |
| `v0.3.2` | 前端重构 Phase 3：活动列表与详情页 |
| `v0.3.3` | 前端重构 Phase 4 收官、时区与签到兑换原子化修复，首次全量生产版本 |
| 历史误命名 `v0.3.0` tag | 与本次重构无关，只保留为历史节点 |
...
```bash
git switch -c fix/from-v0.3.3 v0.3.3
```
