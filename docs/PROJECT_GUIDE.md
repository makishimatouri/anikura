...
| 当前主线状态 | `main` 已包含 PR #1、PR #2、PR #3、PR #4 的文档提交；具体 HEAD 以 `git log --oneline -1` 为准 |
...
- 文档改造开始前，`main` 与 `origin/main` 一致，工作区没有未提交业务代码改动；文档相关 PR #1 至 PR #4 已合并到 `main`，当前文档体系已进入默认分支。
- `npm run check`：通过。
  - `typecheck`：通过。
...
- PR #1 建立基础维护文档，PR #2 建立长期文档更新规范，PR #3 修正状态快照，PR #4 修正文档中的主线状态表述；这些都是文档提交，不改变正式产品版本号。
